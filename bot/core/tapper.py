...
class Tapper:
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        try:
            ...
            logger.warning("<yellow>Invalid REF LINK! (Put in the ref link, not the ref id)</yellow>")
            sys.exit()
        if proxy:
        ...
        actual = random.choices([self.my_ref, ref_param], weights=[40, 60], k=1)
    ...
    async def join_channel(self):
        try:
            logger.info(f"<cyan>{self.session_name}</cyan> | Joining TG channel...")
            if not self.tg_client.is_connected:
                try:
                    await self.tg_client.connect()
                except (Unauthorized, UserDeactivated, AuthKeyUnregistered):
            ...
            try:
                ...
                logger.success(f"<cyan>{self.session_name}</cyan> | <green>Joined channel successfully</green>")
            except Exception as e:
                logger.error(f"<cyan>{self.session_name}</cyan> | <red>Join TG channel failed - Error: {e}</red>")
    ...
    async def check_proxy(self, http_client: aiohttp.ClientSession, proxy: Proxy) -> None:
        try:
            ...
            logger.info(f"<cyan>{self.session_name}</cyan> |🟩 Logging in with proxy IP <green>{ip}</green> and country <cyan>{country}</cyan>")
        except Exception as error:
    ...
    async def login(self, http_client: cloudscraper.CloudScraper):
        try:
            ...
            if login.status_code == 201:
                ...
                logger.success(f"<cyan>{self.session_name}</cyan> | <green>Successfully logged in!</green>")
                return data
            else:
                print(login.text)
                logger.warning(f"<cyan>{self.session_name}</cyan> | Failed to login: <yellow>{login.status_code}</yellow>")
                return None
        except Exception as e:
    ...
    async def get_tasks(self, http_client: cloudscraper.CloudScraper):
        try:
            logger.info(f"<cyan>{self.session_name}</cyan> | Getting tasks list...")
            ...
            if tasks.status_code == 200:
                ...
                logger.warning(f"<cyan>{self.session_name}</cyan> | Failed to get task: <yellow>{tasks.status_code}</yellow>")
                return None
        except Exception as e:
            # traceback.print_exc()
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error while trying to get tasks: {e}")
            return None
    ...
    async def claim_task(self, task, http_client: cloudscraper.CloudScraper, attempt=10):
        ...
        try:
            ...
            logger.info(
                f"<cyan>{self.session_name}</cyan> | Attempt <red>{10 - attempt + 1}</red> to complete task: <cyan>{task['title']}</cyan>")
            tasks = http_client.post(claim_task, json=payload)
            if tasks.status_code == 201:
                res = tasks.json()
                data = res['data']
                if data:
                    logger.success(f"<cyan>{self.session_name}</cyan> | <green>Successfully completed task: <cyan>{task['title']}</cyan> - Earned <cyan>{task['rewards'][0]['amount']}</cyan> paws</green>")
                    return True
                else:
                    logger.info(f"<cyan>{self.session_name}</cyan> | Failed to claim task: {task['title']}, Retrying...")
                    await asyncio.sleep(random.randint(3, 5))
                    return await self.claim_task(task, http_client, attempt - 1)
            else:
                logger.warning(
                    f"<cyan>{self.session_name}</cyan> | <yellow>Failed to complete {task['title']}: {tasks.status_code}</yellow>")
                return await self.claim_task(task, http_client, attempt - 1)
        except Exception as e:
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error while trying to get tasks: {e}, Retrying...")
    ...
    async def proceed_task(self, task, http_client: cloudscraper.CloudScraper, maxattemp, attempt=10):
        ...
        try:
            ...
            logger.info(f"<cyan>{self.session_name}</cyan> | Attempt <red>{maxattemp-attempt+1}</red> to complete task: <cyan>{task['title']}</cyan>")
            ...
            if tasks.status_code == 201:
                ...
                # print(res)
                if data:
                    logger.success(f"<cyan>{self.session_name}</cyan> | <green>Successfully completed <cyan>{task['title']}</cyan></green>")
                    return await self.claim_task(task, http_client, 5)
                else:
                    logger.info(f"<cyan>{self.session_name}</cyan> | Waiting to complete task: <cyan>{task['title']}</cyan>...")
                    await asyncio.sleep(random.randint(5, 10))
                    return await self.proceed_task(task, http_client, maxattemp, attempt-1)
            else:
                logger.warning(f"<cyan>{self.session_name}</cyan> | <yellow>Failed to complete {task['title']}: {tasks.status_code}</yellow>")
                return await self.proceed_task(task, http_client, maxattemp, attempt-1)
        except Exception as e:
            logger.error(f"<cyan>{self.session_name}</cyan> | Unknown error while trying to get tasks: {e}, Retrying...")
            await asyncio.sleep(random.randint(1,3))
            ...
            try:
                if check_base_url() is False:
                    ...
                    if settings.ADVANCED_ANTI_DETECTION:
                        logger.warning(
                            "<yellow>Detected index js file change. Contact me to check if it's safe to continue: https://t.me/cyb3rtati0n</yellow>")
                    else:
                        logger.warning(
                ...
                if can_run:
                            ...
                            if task_list:
                                for task in task_list:
                                    ...
                                    if task['code'] in settings.IGNORE_TASKS:
                                        logger.info(f"<cyan>{self.session_name}</cyan> | Skipped {task['code']} task! ")
                                        continue
                    ...
                    logger.info(f"----<green>Completed <cyan>{self.session_name}</cyan></green>----")
